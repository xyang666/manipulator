...
def build_manifest(checkpoint_root: Path, result_root: Path) -> dict:
    training_jobs = []
    learned = {
    ...
    for method, flags in learned.items():
        for seed in EVALUATION.train_seeds:
            ...
            command = [
                ...
                "--save_path", str(checkpoint_root), *flags,
            ]
            training_jobs.append({"method": method, "seed": seed,
    ...
    for method in PHASE1_METHODS:
        for scenario in PHASE1_SCENARIOS:
            for seed in EVALUATION.train_seeds:
                ...
                checkpoint = checkpoint_root / method / f"seed_{seed}" / "ckpt_best.pt"
                command = [
...
def main() -> int:
    ...
    parser.add_argument("--checkpoint-root", type=Path, default=Path("checkpoints/phase1"))
