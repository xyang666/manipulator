...
train_vanilla.py
----------------
Training entry point for Vanilla SAC baseline.
Direct joint velocity control — no physics-informed components.

Usage:
    code/.venv/bin/python -u code/train_vanilla.py --steps 500000 --n_envs 16

Based on train.py (physics-informed version) with SAC-only, no physics.
"""
...
def parse_args():
    ...
    # Reward args (shared with physics-informed version for fair comparison)
    ...
    p.add_argument("--save_path", type=str, default="checkpoints/vanilla_sac.pt")
...
def main():
    ...
    run_name = args.run_name or f"vanilla_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_dir = os.path.join(os.path.dirname(args.save_path), run_name)
    hyperparams = {
        "steps": args.steps, "batch_size": args.batch_size,
        "start_steps": args.start_steps, "buffer_size": args.buffer_size,
        "d_critical": args.d_critical, "alpha_relax": args.alpha_relax,
    ...
    if args.resume is not None:
        ...
        if os.path.exists(ckpt_path):
            ...
            if (_scene_ema is not None and "scene_ema" in meta
                ...
                print(f"[train] Restored scene stats for {len(_scene_ema)} scenes")
    ...
    if args.render:
        ...
        while total_steps < args.steps:
            ...
            while not done:
                ...
                buffer.push(obs, action, reward_scaled, next_obs, done)
            ...
            ckpt_meta = {
                ...
                "best_reward": logger.best_reward, "hyperparams": hyperparams,
        ...
        while total_steps < args.steps:
            ...
            for i in range(n_envs):
                if total_steps < args.start_steps:
                    actions[i] = np.random.uniform(-0.5, 0.5, ref_env.n)
                else:
